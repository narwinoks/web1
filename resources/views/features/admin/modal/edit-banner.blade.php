...
@php
    $banner = json_decode($data->content, true);
@endphp
    ...
    <div class="modal-body">
        ...
        <div class="mb-1 row justify-content-center text-center">
            @if (isset($data['file']) && file_exists(public_path('assets/img/' . $data['file'])))
                <div id="imagePreview" src="{{ asset('assets/img/' . $data['file']) }}"
                    alt="placeholder image goes here">
                </div>
            @else
                <div id="imagePreview" src="{{ asset('assets/img/600x400.png') }}" alt="placeholder image goes here">
                </div>
            @endif
        ...
        <div class="mb-1">
            ...
            <select class="form-select form-select-sm input-custom" id="category" aria-label=".form-select-sm example"
                ...
                @foreach ($types as $type)
                    <option {{ $banner['category'] == $type['value'] ? 'selected' : '' }} value="{{ $type['value'] }}">
                        {{ $type['label'] }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-1">
            ...
            <textarea type="text" class="form-control input-custom" placeholder="Enter Your title" aria-label="title"
                aria-describedby="basic-addon1" id="title" name="title">{{ $banner['title'] }}</textarea>
